...
async function StreamToString(stream) {
  ...
  return new Promise((resolve, reject) => {
    ...
    .then((data) => {
      return data
    }, () => { // if timeout destroy stream
      stream.destroy()
      throw new Error('read stream timeout')
    })
}
...
async function resetMysql(container, rootPwd, database) {
  await execCommand(container, ['mysql',
    '-uroot',
    `-p${rootPwd}`,
...
async function start(options) {
  ...
  return { rootPwd, port: hostPort, database, stop: cleanUp }
}
